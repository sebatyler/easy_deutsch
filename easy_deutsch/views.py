...
from .models import get_random_row
...
def get_context(word):
    ...
    word_info["images"] = py_.map(res.json()["hits"], lambda x: dict(preview=x["previewURL"], large=x["largeImageURL"]))
...
def home(request):
    ...
    if word:
        ...
        for d in data["data"]:
            d["de_highlight"] = (
                py_(d["de"].split(" "))
                .map(lambda s: f"<font color='red' class='font-weight-bold'>{s}</font>" if s.lower() == word else s)
                .join(" ")
                .value()
            )
...
def sentence(request):
    ...
    data = dict(languages=["de", "en", "es", "fr", "ko"], translations=dict(de=de, en=en), query=dict(de=de, en=en))
    translator = Translator()
...
def random(request):
    row = get_random_row()
    return render(request, "random.html", row)
